...
/* Debug options -------------------------------------------------------------*/
//#define DEBUG_MAIN_INIT_SEQUENCE
...
HAL_StatusTypeDef HAL_InitTick(uint32_t TickPriority)
{
	...
	Period = [(TIM_CLK/1000) - 1]. to have a (1/1000) s time base.
...
/* Disable the tick increment by disabling TICK_TIM update interrupt. */
void HAL_SuspendTick(void)
{
	/* Disable TICK_TIM update Interrupt */
	__HAL_TIM_DISABLE_IT(&hHAL_TickTim, TIM_IT_UPDATE);
}

/* Resume Tick increment. */
/* Enable the tick increment by enabling TICK_TIM update interrupt. */
void HAL_ResumeTick(void)
{
	/* Enable TICK_TIM Update interrupt */
...
__attribute__((weak)) void MainAppTask() {}
__attribute__((weak)) void MainAppProcess() {}
__attribute__((weak)) void MainAppTimeCriticalTask() {}
...
#if defined(DEBUG_MODULES) && defined(DEBUG_MAIN_INIT_SEQUENCE)
	#ifdef TEST_0_GPIO_PORT
		HAL_GPIO_WritePin(TEST_0_GPIO_PORT, TEST_0_GPIO_PIN, GPIO_PIN_SET);
	#endif /*TEST_0_GPIO_PORT*/
#endif /*defined(DEBUG_MODULES) && defined(DEBUG_MAIN_INIT_SEQUENCE)*/
...
static void HeapInit()
{
	...
	/* Heap_5 is used so the maximum heap size can be calculated and initialized
	at run time. */
